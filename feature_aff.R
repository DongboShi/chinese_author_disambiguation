...
# write.csv(GlobalAFF2,file='/Users/zijiangred/changjiang/dataset/feature/feature_Aff/GlobalAFF2.csv',row.names = F,na ='')

GlobalAFF1 <- read_csv('/Users/zijiangred/changjiang/dataset/feature/feature_Aff/GlobalAFF1.csv')
...
for (i in id){
    ...
    colnames(pairorderA) <- c("paperA","paperB","aff","org1","org2","org1_countA","org2_countA")
    pairorderB <- left_join(pairorder,AFF,by=c('paperB'='ut'))
    colnames(pairorderB) <- c("paperA","paperB","aff","org1","org2","org1_countB","org2_countB")
    
    pairorderA_pairorderB_intersectorg1 <- inner_join(select(pairorderA,-aff,-org2,-org2_countA),
                                                      select(pairorderB,-aff,-org2,-org2_countB)) %>% distinct() 
